...
class PhysicsManager
{
  ...
  {
    if((p.pos.x <= -1*width/2 && p.vel.x < 0) || (p.pos.x >= width/2 && p.vel.x > 0)) p.vel.x *= -1; //vertical limits
    if((p.pos.y <= -1*height/2 && p.vel.y < 0) || (p.pos.y >= height/2 && p.vel.y > 0)) p.vel.y *= -1; //horizontal limits    
  }
